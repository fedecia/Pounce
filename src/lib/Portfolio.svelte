<script lang="ts">
  ...
  const snippet = (value = '', max = 88) => {
    const clean = value.trim()
    if (!clean) return 'No thesis logged yet.'
        ...
        <div class="rounded-2xl border border-slate-800 bg-slate-950/70 p-4">
          <div class="flex items-start justify-between gap-4">
            <div>
              ...
              <div class="mt-2 max-w-xl text-sm leading-6 text-slate-300">{snippet(position.journal.thesis)}</div>
            </div>
            <div class="text-right">
